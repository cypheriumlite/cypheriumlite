Cypheriumlite Core version *0.15.1* is now available from:

  <https://download.cypheriumlite.org/cypheriumlite-0.15.1/>
...
A number of changes to the way Cypheriumlite Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 cypher per kB (0.00001 CYL). This is to prepare for dropping the minimum transaction fee to 0.1 cypher per kB (0.0001 CYL) in the next release.
