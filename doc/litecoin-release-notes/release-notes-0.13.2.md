Cypheriumlite Core version 0.13.2 is now available from:

  <https://download.cypheriumlite.org/cypheriumlite-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/cypheriumlite-project/cypheriumlite/issues>
...
updates will be released anymore. Without security updates, using a cypheriumlite
...
ECDSA signatures inside Cypheriumlite transactions now use validation using
...
Previous versions of Cypheriumlite Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Cypheriumlite Core was to raise the default minimum
relay fee.

Cypheriumlite Core 0.13.2 will have a strict maximum size on the mempool. The
...
Cypheriumlite Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running cypheriumlited without having to do any manual configuration.
...
Cypheriumlite Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Cypheriumlite Core will relay transactions with
...
entirely in Cypheriumlite Core 0.13, and it is at this time undecided whether the
...
Cypheriumlite Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Cypheriumlite Core automatically creates a hidden service to listen on, without
manual configuration. Cypheriumlite Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Cypheriumlite Core is listening, and
...
Cypheriumlited can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Cypheriumlite
Core to use floating fees. By default, Cypheriumlite Core will use floating
fees.
...
(default: `0.0002` CYL/kB).

At all times, Cypheriumlite Core will cap fees at `-maxtxfee=<x>` (default:
0.10) CYL.
Furthermore, Cypheriumlite Core will never create transactions paying less than
...
`cypheriumlite.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /cypheriumliterpc>
        ...
        # optional bypass cypheriumlited rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< cypheriumliterpc:<password>
    </Location>
...
Restarting cypheriumlited will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in cypheriumlite.conf
...
cypheriumlite-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/cypheriumlite-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Cypheriumlite Core code base has
...
- `cypheriumlite-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `cypheriumlite-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Cypheriumlite scripting system that in combination with
...
Cypheriumlite transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Cypheriumlite Core.
...
The two phases are distinct in the Cypheriumlite-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Cypheriumlite wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Cypheriumlite.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Cypheriumlite block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of cypheriumcypher they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of cypheriumcypher being spent is exactly
...
- **Increased security for multisig:** Cypheriumlite addresses (both P2PKH addresses
  ...
  believed to be provided by Cypheriumlite's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Cypheriumlite's current ledger (called the UTXO set),
  ...
  note that Cypheriumlite Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Cypheriumlite users to individually opt-in to almost any change in the Cypheriumlite
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Cypheriumlite Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Cypheriumlite
client to Cypheriumlite Core 0.13.2 to signal for segwit support. When a
...
Since Cypheriumlite Core 0.10.0, nodes have defaulted to only relaying and
...
- `cypheriumlite-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `cypheriumlite-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
